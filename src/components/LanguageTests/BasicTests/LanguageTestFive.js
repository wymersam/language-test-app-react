...
export default function LanguageTestFive(scoreFour) {
  ...
  const currentQuestion = useMemo(() => {
    return questionsFive[indexLanguageTest] || {};
  }, [indexLanguageTest]);
  ...
  const progress = useMemo(() => {
    return Math.round((currentQuestionNumber / questionsFive.length) * 100);
  }, [currentQuestionNumber]);

  const nextQuestionTest = useCallback(() => {
    const nextQuestionIndex = id + 1;
    if (nextQuestionIndex < questionsFive.length) {
        ...
        <div className="test-content">
          ...
          <div className="progress-info">
            <p className="question-counter">
              Question {currentQuestionNumber} of {questionsFive.length}
            </p>
          </div>
